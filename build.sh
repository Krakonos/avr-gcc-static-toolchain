...
if hash apt-get 2>/dev/null; then
    ...
    echo nothing will be installed on arch
fi
...
if [ $BUILD_GCC -eq 1 ]; then
	...
	cd obj-avr
	# fixGCCAVR
    [ $FOR_LINUX -eq 1 ] && confMake "$PREFIX_GCC_LINUX" "$OPTS_GCC" #--with-sysroot="$PREFIX_GCC_LINUX" --with-build-sysroot=/
